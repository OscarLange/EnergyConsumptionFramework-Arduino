...
char* retrieve_data(size_t new_mode)
{
    mode = new_mode;
    //lock
    ...
    while(true)
    {
        //wait for write event to unlock
        if(pthread_mutex_lock(&response_mutex) == 0){
            printf("Readable");
            //if smth was written
            if(readable)
            {
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        {
            int size = 4;
            char req_config[5]  = "Freq"; 
            rsp.attr_value.handle = param->read.handle;
            rsp.attr_value.len = size;
            for(int i = 0; i < size; i++)
            {
                rsp.attr_value.value[i] = req_config[i];
            }
        } else if (mode == 1)
        {
            int size = 4;
            char req_config[5]  = "Work"; 
            rsp.attr_value.handle = param->read.handle;
            rsp.attr_value.len = size;
            for(int i = 0; i < size; i++)
            {
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            //unlock mutex because stuff was written
            if(pthread_mutex_lock(&response_mutex) == 0){
                strncpy(config, (char *)param->write.value, param->write.len);
                config_size = param->write.len;
